...
| Security monitoring and response | You're responsible for investigating and resolving incidents for devices that aren't Microsoft Managed Desktop devices.<p>You're responsible for handling Microsoft Managed Desktop - Microsoft Defender Endpoint (MDE) alerts that are resolved by third party applications, bots, and the Cyber Security Operations Center (CSOC). This includes resolved, pending, and unresolved alerts. Microsoft Managed Desktop Security Operations Center (SOC) doesn’t triage these alerts.</p> |
