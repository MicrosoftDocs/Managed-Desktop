...
- [Manual registration for existing devices](/managed-desktop/get-started/device-registration-methods#manual-registration) from the Microsoft Managed Desktop Devices blade.
- [Auto-registration](/managed-desktop/get-started/device-registration-methods#auto-registration) using the kiosk Autopilot group tag.
...
> [!NOTE]
> You'll need to [wipe a device](/managed-desktop/service-description/operations-and-monitoring#device-wipe-with-factory-reset) or [reassign the device profile](/managed-desktop/working-with-managed-desktop/change-device-profile.md) to remove all settings before a device is reassigned to a different user and/or assigned a different device profile. For more information, visit [Policies enforced on kiosk devices (Windows 10/11)](/windows/configuration/kiosk-policies).
...
If there are applications you need for the full kiosk experience, you may add them to the [Modern Workplace Kiosk device profile Enrollment Status Page](/managed-desktop/get-started/esp-first-run.md#enrollment-status-page-settings ) to meet your business needs.
